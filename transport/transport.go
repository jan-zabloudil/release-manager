...
func NewHandler(us model.UserService, os model.SettingsService) *Handler {
	h := Handler{
		Mux:         chi.NewRouter(),
		UserSvc:     us,
		SettingsSvc: os,
	...
	h.Mux.Patch("/organization/settings", h.requireAdminUser(h.setSettings))
	h.Mux.Get("/organization/settings", h.requireAdminUser(h.getSettings))
