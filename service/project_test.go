	...
	testCases := []struct {
	...
	}{
		{
			...
			mockSetup: func(auth *svc.AuthorizationService, settings *svc.SettingsService, user *svc.UserService, github *githubmock.Client, projectRepo *repo.ProjectRepository) {
				...
				user.On("GetForAuth", mock.Anything, mock.Anything).Return(model.User{}, nil)
...
func TestProjectService_ListProjects(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "Non admin user",
			mockSetup: func(auth *svc.UserService, projectRepo *repo.ProjectRepository) {
				auth.On("GetForAuth", mock.Anything, mock.Anything).Return(model.User{
		...
		},
		{
			...
			mockSetup: func(auth *svc.UserService, projectRepo *repo.ProjectRepository) {
				auth.On("GetForAuth", mock.Anything, mock.Anything).Return(model.User{}, svcerrors.NewUnauthorizedUnknownUserError())
			},
			wantErr: true,
		},
	}
